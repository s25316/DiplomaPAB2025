...
namespace Diploma.Infrastructure.RelationalDatabase.MsSql.HighEducations.Courses
{
    public class DisciplineCourseEFConfiguration : IEntityTypeConfiguration<DisciplineCourse>
    {
        ...
        {
            builder.ToTable(nameof(DisciplineCourse));
            builder.HasKey(x => x.DisciplineCourseId).HasName($"{nameof(DisciplineCourse)}_pk");
            builder.Property(x => x.DisciplineCourseId).HasDefaultValueSql("(newid())");
